...
VALUES ('qwer1234@qwer.com', '0123');

-- 이메일 인증 확인
SELECT * FROM email_auth
WHERE email = 'qwer1234@qwer.com'
AND auth_number = '0123';

-- 회원가입
INSERT INTO user (id, password, email)
VALUES ('qwer1234', 'qwer1234', 'qwer1234@qwer.com');

-- 게시물 리스트
SELECT 
	receipt_number, status, title, writer_id, write_date, view_count
FROM board
WHERE title LIKE '%두%'
ORDER BY receipt_number DESC
LIMIT /*(:page_number-1)*10*/0, 10;
...
-- 게시물 상세보기
SELECT
	receipt_number, title, writer_id, write_date, view_count, contents, reply
FROM board
WHERE receipt_number = 1;
