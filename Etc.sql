...
-- CASE
-- 		WHAN 조건1 THEN 조건1이 참일때 결과
-- 		WHAN 조건2 THEN 조건2가 참일때 결과
...
-- IF 함수 : 첫번째 매개개변수로 전달한 표현식에 따라 결과를 반환하는 함수
-- IF (조건, 참일때결과, 거짓일때결과)		-- 자바 ? : 삼항연산자와 비슷한 형식
...
SELECT LENGTH(name) / 3 '글자수' FROM employee;	-- 한글 한문자가 3바이트의 용량이라서 나누기 3을 하여 글자수가 몇개인지 알아봄

-- CONCAT(문자열, ...) : 매개변수로 전달받은 문자열들을 결합
-- 매개변수 중 하나라도 null이면 null을 반환
SELECT CONCAT(사원이름, 부서명) '사원이름+부서명' FROM employee_view2;

-- LEFT(문자열, 길이), RIGHT(문자열, 길이) : 문자열을 왼쪽, 오른쪽에서부터 추출
SELECT LEFT(사원이름, 2) '왼쪽2글자', RIGHT(사원이름, 2) '오른쪽2글자' FROM employee_view2;

-- REPLACE(기준문자열, 찾을문자열, 바꿀문자열) : 특정 문자열을 지정한 문자열로 변경하는 함수
SELECT REPLACE(사원이름, '길동', '순자') '이름' FROM employee_view2;