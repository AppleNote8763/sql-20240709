...
ALTER TABLE employee  -- 테이블 생성후 FOREIGN KEY 지정
ADD CONSTRAINT department_code_fk
FOREIGN KEY (department_code)
REFERENCES department(department_code);

ALTER TABLE employee  -- FOREIGN KEY 지정해제
...
ON E.department_code = D.department_code
WHERE D.name = '영업부';
-- 위의 조건에서의 조회 방식은 조회량이 많아 지기 때문에 서브쿼리를 이용하는게 좋음
...
FROM employee
WHERE department_code = (
	SELECT *	-- *를 쓸려면 WHERE에서 해당 테이블에 모든 컬럼을 조건으로 걸어야함
    FROM department
    WHERE name = '영업부'
);
